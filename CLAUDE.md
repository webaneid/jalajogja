...
- [~] **Event — Check-in via Scan Kamera (QR)** — kode SELESAI (migration `0063_event_checkin_token.sql`, `checkInByTokenAction`, komponen `event-qr-scanner.tsx` pakai `html5-qrcode`), type-check + build bersih. Migration belum jalan di VPS, kode belum di-push/dites end-to-end di browser (admin perlu login). Detail: `docs/arsitektur-event.md` § "Check-in via Scan Kamera (QR)".
...
> Alur akun, 3 level akses, aktivasi akun login: **`docs/arsitektur-akun.md`**
