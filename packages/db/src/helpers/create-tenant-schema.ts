  ...
  await db.transaction(async (tx) => {
    ...
    await tx.execute(sql.raw(`
      CREATE TABLE IF NOT EXISTS "${s}".events (
        ...
        show_attendee_stats     BOOLEAN       NOT NULL DEFAULT false,
        attendee_stats_by       JSONB                  DEFAULT '[]',
        cover_id                UUID          REFERENCES "${s}".media(id) ON DELETE SET NULL,
    ...
    await tx.execute(sql.raw(`
      CREATE TABLE IF NOT EXISTS "${s}".product_variations (
        ...
        sku             TEXT,
        price           NUMERIC(15,2),          -- nullable — kosong = pakai harga produk induk
        public_price    NUMERIC(15,2),
        member_price    NUMERIC(15,2),
        stock           INTEGER       NOT NULL DEFAULT 0,
    ...
    await tx.execute(sql.raw(`
      CREATE TABLE IF NOT EXISTS "${s}".settings (
        ...
        "group"    TEXT        NOT NULL DEFAULT 'general'
                               CHECK ("group" IN ('general','contact','payment','display','mail','notif','website','keuangan','toko','donasi','event','forum','ekosistem')),
