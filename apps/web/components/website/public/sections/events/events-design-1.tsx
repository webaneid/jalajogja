...
function renderCard(e: EventCardData, tenantSlug: string, cardDesign: EventsSectionProps["cardDesign"]) {
  switch (cardDesign) {
    default: return <EventCard event={e} variant="grid" tenantSlug={tenantSlug} />;
  }
}

export function EventsDesign1({ events, tenantSlug, sectionTitle, filterHref, cardDesign }: EventsSectionProps) {
    ...
    <section className="py-10 px-4">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="hidden sm:grid grid-cols-2 lg:grid-cols-3 gap-5">
          {events.map(e => (
            <div key={e.id}>{renderCard(e, tenantSlug, cardDesign)}</div>
          ))}
        </div>
      </div>
    </section>
