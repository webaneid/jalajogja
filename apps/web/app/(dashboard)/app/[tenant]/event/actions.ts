...
// ─── enrollInstallmentPlanAction (public) — Fitur Cicilan Fase B ──────────────
// Daftar tiket event via program cicilan — mirip registerForEventAction, tapi invoice yang
// dibuat pakai totalAmount dari plan (bukan harga tiket langsung) + installmentPlanId, dan
// sekaligus generate baris installment_schedules. Registrasi SELALU "pending" (bukan
// "confirmed") — event_registrations baru "confirmed" saat invoice LUNAS SELURUHNYA (hook
// existing di confirmInvoicePaymentAction/verifySubmittedPaymentAction, tidak disentuh sama
// sekali di sini). Lihat plan cicilan untuk detail keputusan desain.

export type EnrollInstallmentData = {
  planId:         string;
  attendeeName:   string;
  attendeePhone?: string | null;
  attendeeEmail?: string | null;
...
export async function enrollInstallmentPlanAction(
  slug: string,
  data: EnrollInstallmentData,
): Promise<ActionResult<{ registrationId: string; registrationNumber: string; invoiceId: string }>> {
  if (!data.attendeeName.trim())
    return { success: false, error: "Nama peserta wajib diisi." };
  ...
  const [plan] = await db
    ...
    .where(eq(schema.installmentPlans.id, data.planId))
    .limit(1);
  if (!plan)                      return { success: false, error: "Program cicilan tidak ditemukan." };
  if (!plan.isActive)             return { success: false, error: "Program cicilan sedang tidak aktif." };
  if (!plan.isPublished)          return { success: false, error: "Program cicilan tidak tersedia." };
  if (plan.sourceType !== "event" || !plan.sourceId)
    return { success: false, error: "Program cicilan tidak valid." };
  if (!plan.totalAmount || plan.installmentCount < 2)
    return { success: false, error: "Program cicilan belum dikonfigurasi dengan benar." };

  const ticketId = plan.sourceId;

  const [ticket] = await db
    .select()
    .from(schema.eventTickets)
    .where(and(eq(schema.eventTickets.id, ticketId), eq(schema.eventTickets.isActive, true)))
    .limit(1);
  if (!ticket) return { success: false, error: "Tiket tidak ditemukan atau tidak aktif." };

  const [event] = await db
    .select({
      id: schema.events.id, slug: schema.events.slug, title: schema.events.title,
      startsAt: schema.events.startsAt, location: schema.events.location,
    })
    .from(schema.events)
    .where(and(eq(schema.events.id, ticket.eventId as string), eq(schema.events.status, "published")))
    .limit(1);
  if (!event) return { success: false, error: "Event tidak ditemukan atau sudah tidak tersedia." };

  // Resolve identitas (opsional, guest boleh daftar) — pola sama registerForEventAction
  let resolvedMemberId:  string | null = null;
  let resolvedProfileId: string | null = null;
  const session = await auth.api.getSession({ headers: await headers() });
  if (session?.user?.id) {
    const [member] = await publicDb.select({ id: members.id }).from(members)
      .where(eq(members.betterAuthUserId, session.user.id)).limit(1);
    if (member) resolvedMemberId = member.id;
    else {
      const [profile] = await publicDb.select({ id: profiles.id }).from(profiles)
        .where(eq(profiles.betterAuthUserId, session.user.id)).limit(1);
  ...
  try {
    const regNumber  = await generateRegistrationNumber(tenantDb);
    const totalAmount = parseFloat(String(plan.totalAmount));

    const reg = await db.transaction(async (tx) => {
      // Lock tiket — cegah over-book kalau tiket yang sama juga dipakai jalur registrasi biasa
      await tx.select({ id: schema.eventTickets.id }).from(schema.eventTickets)
        .where(sql`${schema.eventTickets.id} = ${ticketId} FOR UPDATE`).limit(1);

      if (ticket.quota != null) {
        const [{ used }] = await tx.select({ used: count() }).from(schema.eventRegistrations)
          .where(and(eq(schema.eventRegistrations.ticketId, ticketId), sql`${schema.eventRegistrations.status} != 'cancelled'`));
        if (Number(used) >= ticket.quota) throw new Error("Kuota tiket sudah habis.");
      }

      const [inserted] = await tx.insert(schema.eventRegistrations).values({
        registrationNumber: regNumber,
        eventId:            event.id,
        ticketId,
        memberId:           resolvedMemberId,
    ...
    // Term 1 jatuh tempo hari ini, termin berikutnya +intervalDays kelipatan.
    const perTerm  = Math.round(totalAmount / plan.installmentCount);
    const lastTerm = totalAmount - perTerm * (plan.installmentCount - 1); // serap sisa pembulatan
    const today    = new Date();

    const { invoiceId, invoiceNumber, uniqueCode } = await createLinkedInvoice(tenantDb, {
      sourceType:    "event_registration",
      sourceId:      reg.id,
      customerName:  data.attendeeName.trim(),
      customerPhone: data.attendeePhone?.trim() ?? null,
      ...
      items: [{
        itemType:  "ticket",
        itemId:    ticketId,
        name:      `${plan.name} (${ticket.name})`,
        unitPrice: totalAmount,
        quantity:  1,
      }],
    });

    const scheduleRows = Array.from({ length: plan.installmentCount }, (_, i) => {
    ...
    await db.insert(schema.installmentSchedules).values(scheduleRows);
    ...
    void (async () => {
      ...
      void notifyWa({
        ...
        vars: {
          ...
          amount:        waRupiah(totalAmount + uniqueCode),
          dueDate:       today.toISOString().slice(0, 10),
          invoiceUrl,
        },
      });
    ...
    revalidatePath(`/app/${slug}/finance/billing/cicilan/${plan.id}`);
    return { success: true, data: { registrationId: reg.id, registrationNumber: regNumber, invoiceId } };
  } catch (err) {
    if (err instanceof Error && err.message.includes("Kuota"))
      return { success: false, error: err.message };
    console.error("[enrollInstallmentPlanAction]", err);
    return { success: false, error: "Gagal mendaftar program cicilan. Silakan coba lagi." };
  }
}
