...
export default async function NotaDinasPage({
  ...
  const letters = await tenantDb
    ...
    .from(schema.letters)
    .where(eq(schema.letters.type, "internal"))
    .orderBy(desc(schema.letters.createdAt));

  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-xl font-semibold">Nota Dinas</h1>
        <p className="text-sm text-muted-foreground mt-0.5">{letters.length} nota dinas</p>
      </div>
