...
// SEO Fase 3 (docs/arsitektur-seo.md § 3.3) — halaman ini belum punya filter kategori/tag
// (lihat catatan Fase 2), jadi override-nya cukup 1 level (page-wide), bukan per-kategori.
export async function generateMetadata({ params }: { params: Params }): Promise<Metadata> {
  const { tenant: slug } = await params;
  const base = await getTenantSeoBase(slug);
  const override = await getPageSeoOverride(createTenantDb(slug), slug, "post-archive");
  return buildMetadata({
    title:         override?.metaTitle || "Berita & Artikel",
    ...
    canonicalUrl:  `${base.baseUrl}/post`,
