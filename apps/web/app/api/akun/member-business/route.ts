...
export async function POST(req: NextRequest) {
  ...
  const valid = processed.filter(e => e.name?.trim() && e.category && e.sector);

  try {
    await db.transaction(async (tx) => {
      await tx.delete(memberBusinesses).where(eq(memberBusinesses.memberId, member.id));
      ...
      for (const e of valid) {
        ...
        if (e.addressCountry || e.addressProvinceId || e.addressDetail) {
          ...
          const [a] = await tx.insert(addresses)
            .values({
        ...
        if (e.instagram || e.facebook || e.linkedin || e.twitter || e.youtube || e.tiktok || e.website) {
          const [s] = await tx.insert(socialMedias)
            .values({
