...
## [2026-09-13] OTP dikirim ke nomor yang diketik client sendiri bukan bukti kepemilikan identitas yang diklaim
**Masalah:** Audit keamanan bertahap (Fase 3) menemukan varian BARU dari celah identity-takeover yang sudah pernah difix di `lib/akun-identity.ts` (lesson `[2026-07-31]` di atas) — kali ini lewat jalur klaim-by-ID. `GET /api/akun/lookup-member?stambuk=` (publik) mengembalikan `memberId` untuk siapa pun yang tahu/menebak nomor stambuk. `POST /api/akun/register` mode `claimMemberId` hanya cek member belum punya akun + email belum dipakai — TIDAK PERNAH verifikasi pendaftar benar pemilik identitas itu. OTP-nya sendiri (kalau WA gateway aktif) dikirim ke nomor HP yang **diketik bebas oleh pendaftar di form** (`phone` state client), bukan ke `contacts.phone`/`whatsapp` milik member yang diklaim — jadi OTP tidak pernah membuktikan apa pun. Kalau WA gateway tenant belum aktif, klaim lolos tanpa verifikasi sama sekali.
**Root cause:** Fix `[2026-07-31]` menutup jalur auto-link by email-match, tapi jalur klaim-by-ID (`claimMemberId`, sudah ada lebih dulu) tidak pernah diaudit dengan pertanyaan yang sama: "apakah OTP di sini benar-benar mengonfirmasi kepemilikan nomor yang TERCATAT di identitas yang diklaim, atau cuma nomor yang diketik user saat itu?" Dua implementasi verifikasi identitas berbeda di file yang berbeda, satu terlanjur benar (auto-link direvert total), satu lolos sampai audit menyeluruh.
**Fix:** `/api/akun/send-otp` dan `/api/akun/verify-otp` — kalau `type=register` + `claimMemberId` terisi, resolve nomor OTP dari `members.contactId → contacts.whatsapp/phone` SERVER-SIDE, abaikan total `phone` dari body request. `verify-otp` menerbitkan `claimToken` sekali-pakai (tabel `verification`, TTL 10 menit) setelah OTP cocok — `/api/akun/register` WAJIB validasi `claimToken` ini (via `DELETE ... RETURNING` atomic, bukan SELECT-lalu-DELETE terpisah supaya tidak race) sebelum lanjut ke mutasi `members`, dan klaim tanpa `claimToken` valid ditolak tanpa pengecualian — tidak lagi tunduk ke toggle admin "OTP aktif/tidak" yang sebelumnya bisa membuat klaim lolos tanpa verifikasi apa pun.
**Pencegahan:** Kalau ada DUA cara identitas bisa "diklaim" (email-match vs ID-langsung, atau kombinasi lain), fix di satu jalur TIDAK otomatis menutup jalur lain — audit ulang keduanya dengan pertanyaan yang sama. OTP/kode verifikasi apa pun HARUS dikirim ke kontak yang tercatat di sistem untuk identitas yang diklaim, TIDAK PERNAH ke kontak yang diketik bebas oleh pihak yang sedang mengklaim — kalau tidak, OTP cuma teater keamanan (mengonfirmasi "pendaftar bisa terima WA", bukan "pendaftar adalah pemilik identitas ini"). Proof-of-verification yang dipakai lintas-request (OTP tervalidasi di step 1 → dipakai di step 2 mutasi) wajib token sekali-pakai yang dikonsumsi via satu statement DB atomic (`DELETE...RETURNING`), bukan flow control client-side semata (client bisa saja panggil endpoint step 2 langsung, skip step 1 sama sekali).

---

## [2026-09-13] `bun audit | tail -N` bisa memotong temuan; `bun audit` sendiri bisa false-positive di dependency transitif ter-dedup
**Masalah:** Audit keamanan bertahap sesi ini awalnya melaporkan "62 vulnerabilities" ke user berdasarkan `bun audit 2>&1 | tail -60` — ternyata OUTPUT ASLINYA lebih panjang dari 60 baris, `tail -60` diam-diam memotong bagian AWAL laporan. Setelah dijalankan ulang tanpa `tail`, muncul temuan yang sebelumnya tidak terlaporkan sama sekali ke user, termasuk **1 CRITICAL di `better-auth`** (OAuth refresh-token replay) yang sempat lolos dari laporan pertama. Terpisah, setelah upgrade `sharp` ke versi aman, `bun audit` MASIH melaporkan `sharp <0.35.0` vulnerable "via next" — investigasi manual ke `bun.lock` membuktikan hanya ADA SATU versi sharp ter-resolve (yang sudah aman), jadi ini false-positive: `bun audit` tampaknya mencocokkan declared range di package.json (Next.js mendeklarasikan `sharp: "^0.34.3 || ^0.35.3"` sebagai optionalDependency) terhadap advisory, bukan versi yang BENAR-BENAR ter-lock/resolve.
**Root cause:** (1) Kebiasaan pipe `| tail -N` ke command yang outputnya tidak diketahui panjangnya sebelumnya — aman untuk command dengan output pendek yang sudah dikenal, berbahaya untuk audit/scan tool yang volumenya tidak terprediksi. (2) `bun audit` tidak selalu membedakan "range yang diizinkan mengandung versi vulnerable" vs "versi yang benar-benar terkunci di lockfile vulnerable" — perlu cross-check manual ke `bun.lock` (grep `"nama-package":\s*\[` untuk versi yang benar-benar resolve) sebelum menyimpulkan sebuah temuan itu nyata.
**Fix:** Selalu jalankan `bun audit` TANPA `tail`/`head` kalau tidak yakin panjang outputnya (redirect ke file kalau perlu dibaca ulang, jangan potong langsung di pipe). Untuk tiap temuan yang "curiganya" masih muncul setelah upgrade, cross-check `grep -n '"<package>":\s*\[' bun.lock` — kalau resolusi lockfile-nya sudah versi aman, itu false-positive (biasanya karena tool audit mencocokkan manifest range, bukan lockfile).
**Pencegahan:** Jangan pipe command audit/scan ke `tail`/`head` tanpa tahu dulu berapa panjang outputnya. Kalau `bun audit` (atau tool sejenis) masih melaporkan sebuah temuan setelah versi yang dipakai sudah dinaikkan, verifikasi dulu ke lockfile sebelum melapor ke user sebagai "belum fixed" — bisa jadi false-positive by design tool-nya.

## [2026-09-13] Root vs `apps/web` package.json — deklarasikan dependency di workspace yang benar-benar memakainya
**Masalah:** Beberapa runtime dependency (`@aws-sdk/client-s3`, `@radix-ui/react-switch`, `lucide-react`, `jsqr`, dll) genuinely dipakai kode `apps/web` tapi cuma terdaftar di `dependencies` root `package.json` — lolos build lokal karena Bun workspace hoisting, tapi salah tempat (root harusnya cuma untuk tooling lintas-workspace kayak `turbo`).
**Root cause:** `bun add <pkg>` dijalankan dari root repo tanpa `--cwd apps/web` atau `-F @jalajogja/web`, jadi entry masuk ke root `package.json` alih-alih workspace yang benar.
**Fix:** Pindahkan ke `dependencies`/`devDependencies` `apps/web/package.json` (atau `packages/db/package.json` kalau memang dipakai di situ), install ulang, `type-check` + `build` full untuk verifikasi.
**Pencegahan:** Kalau menambah dependency baru untuk kode yang hidup di `apps/web`, selalu `bun add <pkg> --cwd apps/web` (bukan `bun add <pkg>` polos dari root) — kecuali dependency itu memang lintas-workspace (dipakai `packages/db` juga).

---

## [2026-09-13] `content` Tiptap tersimpan sebagai JSON bebas dari client — wajib disanitasi di titik RENDER, bukan diasumsikan trusted karena "dari editor sendiri"
**Masalah:** `renderBody()` (`lib/letter-render.ts`, dipakai render post/halaman/surat publik) merender `embedBlock.attrs.html` mentah via string interpolasi langsung ke HTML output (`<div>${html}</div>`), dan `link` mark meng-escape karakter HTML tapi tidak memvalidasi skema `href` (`javascript:` lolos). Server Action `updatePostAction`/`createPostAction` menerima `content` sebagai JSON bebas dari client TANPA validasi struktur apa pun — siapa pun dengan akses "website: full" di suatu tenant (termasuk custom role yang mungkin kurang dipercaya daripada owner) bisa memanggil action langsung (bypass UI editor) dengan payload berisi script/javascript: href, tereksekusi untuk SEMUA pengunjung publik tenant itu.
**Root cause:** Asumsi implisit "konten yang lewat editor Tiptap sendiri pasti aman" — padahal jalur masuknya adalah Server Action yang menerima JSON mentah, bukan DOM Tiptap yang sudah di-sanitize browser. Skema HTML dari oEmbed pihak ketiga (`embed-block-ext.ts`, YouTube/Instagram/noembed.com) makin memperkuat asumsi salah ini karena kontennya "terlihat" datang dari sumber terpercaya, padahal disimpan lewat jalur yang sama persis dengan input arbitrary.
**Fix:** `sanitizeEmbedHtml()` (whitelist tag/attribute via `sanitize-html`, plus host-whitelist khusus untuk `iframe`/`script` karena dua tag itu satu-satunya yang bisa eksekusi kode — scheme-whitelist saja tidak cukup untuk keduanya) dan `sanitizeLinkHref()` (whitelist skema `http/https/mailto/tel`/relatif) dipanggil di titik RENDER (`renderBody`), bukan di titik save — otomatis menutup konten lama yang sudah tersimpan sebelum fix ini juga, tidak perlu migration data.
**Pencegahan:** Kalau ada field yang disimpan sebagai "rich content" (JSON Tiptap, HTML, Markdown) dari Server Action yang menerima payload client secara longgar (tanpa Zod/schema validation, pola umum di project ini) — JANGAN asumsikan aman karena "cuma bisa diisi lewat editor kami sendiri". Validasi/sanitasi di titik RENDER publik, karena itu satu-satunya titik yang menjamin cakupan penuh (data lama + baru, jalur UI + jalur langsung ke Server Action).

## [2026-09-13] Action baru di file `actions.ts` yang sudah punya banyak sibling actions — WAJIB disamakan pattern authorization-nya, mudah kelewat kalau ditambah belakangan
**Masalah:** `saveVariationsAction`/`generateVariationsAction` (`toko/actions.ts`) cuma cek `getTenantAccess(slug)` — tidak ada `hasFullAccess(access.tenantUser, "toko")` yang konsisten dipanggil di 13 sibling action lain di file yang sama persis. Role dengan akses toko `"none"`/`"read"` (mis. bendahara) bisa create/update/delete variasi produk termasuk harga & stok, padahal harusnya tidak berwenang.
**Root cause:** Kedua action ini kemungkinan ditambahkan di iterasi terpisah (fitur "Variasi Produk" datang belakangan dari fitur produk dasar) tanpa proses eksplisit "copy pattern dari action lain di file yang sama" — gampang lolos karena secara fungsional kode tetap jalan normal untuk user yang authorized, cuma tidak menolak yang tidak authorized.
**Fix:** Tambah baris `if (!hasFullAccess(access.tenantUser, "toko")) return { error: "Akses ditolak." };` persis setelah `getTenantAccess`, sama seperti sibling actions.
**Pencegahan:** Setiap kali menambah Server Action baru ke file `actions.ts` yang sudah berisi banyak actions lain untuk modul yang sama — WAJIB grep dulu pattern authorization yang dipakai sibling actions di file itu (`grep -n "hasFullAccess\|hasReadAccess\|canAccess" <file>`) dan samakan persis, jangan cuma pastikan `getTenantAccess` ada. Kalau review/audit menemukan SATU action tanpa permission check di file yang isinya konsisten pakai permission check, itu sinyal kuat ada action lain yang lolos juga — cek semua action di file yang sama, bukan cuma yang ditemukan pertama.

## [2026-09-13] Auth-check dengan pola `if (SECRET) { cek header }` fail-OPEN kalau env var kosong — selalu fail-closed
**Masalah:** `GET /api/cron/verify-domains` membungkus pengecekan `CRON_SECRET` dalam `if (CRON_SECRET) { ... }` — kalau env var tidak diset di suatu environment, blok pengecekan dilewati SELURUHNYA dan endpoint jadi publik tanpa auth apa pun (bisa dipicu siapa saja, membocorkan daftar custom domain tenant + status DNS-nya). 8 route cron lain di project yang sama sudah benar: `if (header !== process.env.CRON_SECRET) return 401` — fail-closed, karena `header !== undefined` tetap `true` kecuali header literal string `"undefined"`.
**Root cause:** Pola `if (SECRET) { ... }` ditulis dengan asumsi "kalau belum di-set berarti mode dev, lewatkan saja" — asumsi yang masuk akal untuk local dev TAPI berbahaya kalau env var itu genuinely lupa di-set di production (tidak ada sinyal error yang memberi tahu, endpoint diam-diam terbuka).
**Fix:** Ganti ke pola fail-closed yang sama dengan 8 route cron lain: `if (req.headers.get("authorization") !== \`Bearer ${CRON_SECRET}\`) return 401` — tanpa syarat pembungkus apa pun.
**Pencegahan:** JANGAN PERNAH tulis auth-check dengan pola `if (secretEnvVar) { cek }` — itu berarti "env var kosong = lewati pengecekan = akses terbuka". Selalu bandingkan header/token ke `process.env.X` secara LANGSUNG tanpa pembungkus kondisional, supaya env var kosong menghasilkan mismatch (ditolak), bukan bypass. Kalau menulis endpoint cron/internal baru, samakan persis pattern-nya dengan endpoint cron existing di project (grep dulu, jangan reimplementasi dari nol).

---

## [2026-09-13] `displayAuthorId = null` yang didesain "fallback ke Tim Redaksi" ternyata jatuh ke fallback LAIN yang tidak terlihat dari titik penulisan kode
**Masalah:** `docs/arsitektur-import-export-post-wordpress.md` § 2.4 mendesain: kalau nama penulis WordPress tidak bisa di-resolve, `displayAuthorId` dibiarkan `null` dan "byline publik jatuh ke fallback default sistem ('Tim Redaksi')". Yang TIDAK diperiksa saat menulis desain ini: `post-detail-view.tsx` (komponen render, ditulis terpisah untuk kebutuhan post BIASA, bukan spesifik untuk import) punya fallback chain-nya SENDIRI untuk `displayAuthorId = null` — yaitu resolve ke `posts.authorId` (untuk post biasa ini masuk akal: itu memang admin yang menulis post itu). Untuk post HASIL IMPORT, `authorId` bukan penulis, melainkan admin yang menjalankan proses import — tapi komponen render tidak tahu/tidak peduli asal-usul post-nya, jadi tetap fallback ke situ. Hasilnya: alih-alih "Tim Redaksi", yang tampil adalah nama+foto admin importer di semua post yang gagal resolve penulis (dikonfirmasi nyata: 480/480 post di satu tes live).
**Root cause:** Desain arsitektur ditulis dengan asumsi "kalau field X kosong, sistem PASTI fallback ke Y" tanpa menelusuri KODE AKTUAL komponen render yang mengonsumsi field itu — asumsi tersebut tercatat sebagai fakta desain, bukan diverifikasi. Komponen render (`post-detail-view.tsx`) sendiri punya logic yang BENAR untuk use-case aslinya (post biasa) tapi salah kalau dipakai lintas-konteks (post hasil import) tanpa disesuaikan.
**Fix:** `resolveOrCreateAuthor()` diubah supaya TIDAK PERNAH mengembalikan `null` karena nama kosong — fallback ke `post_authors` generik ("Tim Redaksi", find-or-create sama seperti penulis asli) SEBELUM ditulis ke `displayAuthorId`, bukan mengandalkan komponen render untuk fallback yang benar. Prinsipnya: kalau ada beberapa lapis fallback (data layer → render layer), pastikan lapis yang PALING TAHU KONTEKS (di sini: proses import, yang tahu `authorId`-nya cuma audit trail) yang menutup gap-nya, jangan berharap lapis render generik menebak konteks itu.
**Pencegahan:** Kalau menulis dokumen arsitektur yang mengklaim "field X kosong → fallback ke Y" — WAJIB verifikasi ke kode AKTUAL komponen yang mengonsumsi field X itu (baca fungsi resolusinya baris-per-baris), jangan asumsikan dari nama fallback yang "terdengar benar". Kalau ada dua konteks berbeda yang sama-sama menulis ke field yang sama (post biasa vs post hasil import, keduanya menulis `posts.authorId`/`displayAuthorId`) tapi SEMANTIK field itu beda antar konteks (authorId = penulis vs authorId = admin importer) — commit ke fallback yang aman di titik PALING SPESIFIK (proses import), jangan biarkan komponen render generik yang menebak.

## [2026-09-15] `pickupMapsUrl` dari checkout PUBLIK (tanpa auth) ditulis mentah ke DB lalu dirender sebagai `<a href>` — javascript: URI stored-XSS ke admin/mitra
**Masalah:** `checkoutAction` (`cart/actions.ts`, dipanggil dari `/keranjang/checkout` — publik, tanpa login) menerima `CheckoutShippingLine.pickupLocationName`/`pickupAddress`/`pickupMapsUrl` dari client dan menulisnya langsung ke `invoice_shipping_lines` tanpa validasi apa pun (beda dari `deliveryMethod`/`paymentMethod`/`codEnabled`/`pickupEnabled` yang SUDAH di-re-verify server-side terhadap `mitras`/`tokoSettings` di transaction yang sama). `pickupLocationName`/`pickupAddress` aman (dirender sebagai teks JSX biasa, auto-escape React) — tapi `pickupMapsUrl` dirender sebagai `href={sl.pickupMapsUrl}` mentah di 5 tempat: dashboard admin (`invoice-detail-client.tsx`), invoice publik customer (`invoice-public-client.tsx`), halaman pesanan mitra (`pesanan-client.tsx`), dan preview checkout itu sendiri (`checkout-form.tsx`, 2 titik). Siapa pun (anonim) yang checkout dengan grup "Ambil Sendiri" bisa isi `pickupMapsUrl: "javascript:..."` lewat panggilan langsung ke Server Action (bypass UI) — script itu tereksekusi di browser admin/mitra yang klik "Buka di Google Maps →" saat mereka login.
**Root cause:** Field pengiriman (COD/pickup) sejak awal didesain dengan model trust "boolean/enum di-re-verify server, tapi field TEKS (nama/alamat/URL lokasi) dianggap aman karena 'cuma ditampilkan'" — asumsi ini benar untuk teks biasa (React auto-escape) tapi SALAH khusus untuk field yang dipakai sebagai `href`, karena skema URL (`javascript:`/`data:`) tidak "di-escape" oleh React, cuma isi teksnya. Ditemukan saat security review menyeluruh sebelum push 3 fitur pengiriman per-produk baru — fitur "Lokasi Ambil Sendiri per-produk" memperbesar permukaan (field sekarang legitimate berasal dari 3 sumber: produk/mitra/toko) tapi gap trust-nya sendiri sudah ada sejak fitur COD & Ambil Sendiri awal (§ 14 `docs/arsitektur-billing.md`).
**Fix:** `lib/safe-url.ts` — `isSafeExternalUrl()` pakai konstruktor `URL` (bukan `startsWith` string, supaya tidak dilewati trik whitespace/karakter kontrol) untuk memastikan protokol `http:`/`https:` saja. Diterapkan DUA lapis: (1) titik tulis (`checkoutAction` + `createOrderAction`) — URL yang lolos validasi disimpan, selainnya di-null-kan; (2) titik render (kelima file di atas) — guard `isSafeExternalUrl(x) &&` sebelum render `<a href>`, defense-in-depth untuk baris data lama yang mungkin sudah tersimpan sebelum fix ini (juga menutup baris manapun yang lolos dari jalur lain di masa depan).
**Pencegahan:** Kalau ada field TEKS dari input yang tidak sepenuhnya dipercaya (client checkout publik, atau apa pun yang bisa dipanggil langsung ke Server Action tanpa lewat UI) yang NANTINYA dipakai sebagai `href`/`src`/URL redirect — jangan asumsikan "field teks otomatis aman karena React escape". Validasi skema URL (`isSafeExternalUrl()`) WAJIB di titik simpan DAN titik render setiap kali field semacam itu dipakai sebagai atribut URL, bukan cuma di satu titik. Kalau audit menemukan satu titik render `href={x}` tanpa guard, grep semua occurrence nama field yang sama (`grep -rn "<fieldName>"`) — kemungkinan besar ada beberapa titik render paralel yang sama-sama lolos (di sini ada 5).
